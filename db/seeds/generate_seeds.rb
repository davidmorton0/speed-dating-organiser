...
class GenerateSeeds
  def call
    ...
    event = Event.create(title: 'Drinks at a bar', starts_at: DateTime.current + 3.days, rep: rep_1,
    ...
    event = Event.create(title: 'Drinks at a pub', starts_at: DateTime.current + 5.days, rep: rep_1,
    ...
    event = Event.create(title: 'Drinks in a car park', starts_at: DateTime.current + 3.days, rep: rep_2,
                         organisation: organisation, max_rounds: Constants::MAX_ROUNDS)
    create_list(:dater, 4, :female, event: event)
    create_list(:dater, 5, :male, event: event)

    # Create a event with no rep
    event = Event.create(title: 'Dance party', starts_at: DateTime.current + 3.days, rep: nil, organisation: organisation,
                         max_rounds: Constants::MAX_ROUNDS)
    create_list(:dater, 7, :female, event: event)
    create_list(:dater, 5, :male, event: event)

    # Create event with matches
    event = Event.create(title: 'Dating event with matches', starts_at: DateTime.current, rep: rep_2,
                         organisation: organisation, max_rounds: Constants::MAX_ROUNDS)
    ...
    event = Event.create(title: 'Drinks at another bar', starts_at: DateTime.current + 2.days, rep: rep_3,
